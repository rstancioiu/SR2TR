...
/***
	Purpose: Computes the minimum weight matching in a weigthed bipartite
			 graph
...
class Hungarian{	
	...
	private:
		...
		*/
		...
		/**
			The algorithm hopcroft karp that checks if for a given bipartite graph,
		...
		*/
		...
		/**
			...
			the right side R. U the set of unmatched vertexes from the L. Z is the 
			set of vertexes that are either in U or are connected with U by 
			alternating paths:
			K = (L\Z) \cup (R \cap Z).
		...
		*/
		...
		/**
			Transforms the cost matrix based on the last minimum set cover V found:
			Let delta = min(cost_matrix[i][j]) where i,j are not in the V.
			For each element of the matrix we have the following transformations:
			cost[i][j] = | cost[i][j] - delta, if i,j not in V
					     | cost[i][j] 	     , if one of i,j is in V, but not both
					     | cost[i][j] + delta, if both i,j are in V
		*/
