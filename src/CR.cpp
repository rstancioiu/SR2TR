...
void CR::compute_classical_adom(Relation& r, Relation& s) {
	...
	for(uint32_t i = 0; i < n + m; ++i) {
		...
		for(uint32_t j = 1; j < classicalAttrAdom[i].size(); ++j) {
			if(classicalAttrAdom[i][j].first != classicalAttrAdom[i][j-1].first) {
				k++;
				classicalAttrAdom[i][k] = classicalAttrAdom[i][j];
			} else {
				classicalAttrAdom[i][k].second = max(classicalAttrAdom[i][k].second, classicalAttrAdom[i][j].second);
			}
		}
		k++;
...
int CR::binary_search_canonical_high(double a, uint32_t attribute) {
	...
	if(high+1<canonicalAttrAdom[attribute].size() && canonicalAttrAdom[attribute][high+1]==a) high++;
	return high;
}
...
void CR::print_cr() {
    ...
    cout<<"--------------------------------------------------"<<endl;
    for(uint32_t i = 0; i < adom.size(); ++i) {
    	cout<<"["<<setw(4)<<adom[i].first<<";"<<setw(5)<<adom[i].second<<"] || "<<cr[i].toString()<<endl;
    }
    cout<<endl;
}
