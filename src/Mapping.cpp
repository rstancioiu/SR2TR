...
vector<pair<int,int> > Mapping::SR2TR(Relation& r,Relation& s, int satisfaction){
	if(r.size()==0) { n = 0; }
	else { n = r[0].size(); }
	if(s.size()==0) { m = 0; }
	else { m = s[0].size(); }
	...
	pair<double, vector<pair<int,int> > > matching = FindMatching(M);
...
pair<double, vector<pair<int,int> > > Mapping::FindMatching(vector<vector<double> > &M) {

	Hungarian* hungarian = new Hungarian(M);

	pair<double, vector<pair<uint32_t, uint32_t> > > res = hungarian->compute_hungarian();
