...
std::strong_ordering Bitset::operator<=>(
    const Bitset &bitset_to_compare) const {
    uint32_t nb_bits_in_bitset = count();
    uint32_t nb_bits_in_bitset_to_compare = bitset_to_compare.count();
    if (nb_bits_in_bitset == nb_bits_in_bitset_to_compare) {
        for (uint32_t index = 0; index < array.size(); index++) {
            if (array[index] != bitset_to_compare.array[index]) {
                return (array[index] <=> bitset_to_compare.array[index]);
            }
        }
    }
